...
try {
    const stickerModule = await import('./commands/sticker.js').catch(() => null);import fs from 'fs';
import process from 'process';
import os from 'os';
import path from 'path';
import { downloadMediaMessage } from '@whiskeysockets/baileys';
...
async function autonomousWitaSchedulerTick(sock) {
    ...
    try {
        ...
        for (const session of cachedSessions) {
            ...
            const closeEventKey = `close_${todayDateStr}_${sessionId}_${closeStr}`;
            // Skenario 4 HANYA boleh disiarkan dalam jendela toleransi 2 menit pasca batas penutupan
            // Mencegah siaran penutupan palsu (false-positive) saat bot dinyalakan ulang di luar jam sesi
            const isClosingJustNow = (currentTotalMin >= closeMin && currentTotalMin <= closeMin + 2);
            if (isClosingJustNow) {
                if (!sentEventsToday.has(closeEventKey)) {
                    const sent = await broadcastScenarioToStudents(sock, 4, {
